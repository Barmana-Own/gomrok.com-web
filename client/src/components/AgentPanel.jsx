...
export default function AgentPanel({ user, token, apiUrl, onLogout }) {
  ...
  return <main className="agent-shell" dir="rtl"><header className="platform-header"><div className="platform-brand"><span className="platform-brand__mark">Z</span><span><strong>GOMROK</strong><small>AGENT / Z · DESTINATION EVIDENCE</small></span></div><div className="platform-header__user"><span>Agent/Z · {dashboard?.organization?.name || user?.organizationId || '—'}</span><button type="button" onClick={onLogout}>خروج</button></div></header><div className="agent-main"><div className="platform-hero"><div><span className="platform-eyebrow">ASSIGNED DELIVERY CONTROL</span><h1>تحویل مقصد با شواهد قابل دفاع</h1><p>احراز گیرنده، تطبیق CMR، مکان، OTP و POD در یک زنجیره نسخه‌دار برای Review شرکت X.</p></div><div className="platform-hero__status"><i /> KYC: {stateLabel(kycState)}<small> · Device: {deviceReady ? 'bound' : 'pending'}</small></div></div><nav className="agent-nav" aria-label="منوی Agent">{menu.map(([key, label]) => <button type="button" className={section === key ? 'is-active' : ''} key={key} onClick={() => { setSection(key); if (['delivery', 'evidence', 'receipts', 'discrepancies', 'claims', 'authority'].includes(key) && selectedId && !selected) loadTrip(selectedId, key); }}>{label}</button>)}</nav><Notice notice={notice} />{section === 'dashboard' && <><div className="platform-metrics"><article><span>تحویل‌های تخصیص‌یافته</span><strong>{dashboard?.metrics?.deliveries || 0}</strong><small>Assignment scoped</small></article><article><span>در انتظار اقدام</span><strong>{dashboard?.metrics?.waiting || 0}</strong><small>Verification / POD</small></article><article><span>شاهد ناقص یا برگشتی</span><strong>{dashboard?.metrics?.incompletePod || 0}</strong><small>Controlled review</small></article></div><Card title="صف تحویل مقصد" eyebrow="PENDING DELIVERIES" actions={<div className="agent-card-actions"><select className="agent-filter" value={deliveryFilter} onChange={(event) => setDeliveryFilter(event.target.value)} aria-label="فیلتر تحویل"><option value="ALL">همه تحویل‌ها</option>{deliveryFilters.slice(1).map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select><button className="platform-button" type="button" disabled={busy} onClick={loadDashboard}>به‌روزرسانی</button></div>}>{renderDeliveryCards()}</Card>{dashboard?.notifications?.length ? <Card title="اعلان‌ها" eyebrow="DOMAIN EVENTS"><div className="agent-notification-list">{dashboard.notifications.slice(0, 8).map((item) => <span key={item.id}>{item.payload?.eventName || 'رویداد'} · {item.created_at}</span>)}</div></Card> : null}</>}{section === 'finance' && renderFinance()}{section === 'notifications' && <Card title="اعلان‌ها" eyebrow="AUDITED NOTIFICATIONS"><div className="agent-notification-list">{(dashboard?.notifications || []).map((item) => <span key={item.id}>{item.payload?.eventName || 'رویداد'} · {item.created_at}</span>)}</div></Card>}{section === 'security' && renderProfile()}{['delivery', 'evidence', 'receipts', 'discrepancies', 'claims', 'authority'].includes(section) && renderDetail()}</div></main>;
}
