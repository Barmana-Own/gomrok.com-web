...
export default function DriverMobilePanel({ user, token, apiUrl, onLogout }) {
  ...
  const [cargoOpportunities, setCargoOpportunities] = useState([]);
  ...
  const [cargoOffer, setCargoOffer] = useState(null);
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const [nextHome, nextProfile, nextOpportunities, nextTrips, nextSettlements, nextCargoOpportunities] = await Promise.all([
        api('/api/platform/driver/dashboard'), api('/api/platform/driver/profile'), api('/api/platform/driver/opportunities'), api('/api/platform/driver/trips'), api('/api/platform/driver/settlements'), api('/api/cargo-inquiries/driver/opportunities').catch(() => ({ items: [] }))
      ]);
      setHome(nextHome); setProfile(nextProfile); setOpportunities(nextOpportunities.opportunities || []); setCargoOpportunities(nextCargoOpportunities.items || []); setTrips(nextTrips.trips || []); setSettlements(nextSettlements.settlements || []);
  ...
  const submitCargoOffer = async () => {
    if (!cargoOffer) return;
    const amount = Number(cargoOffer.amount);
    if (!Number.isFinite(amount) || amount <= 0) return setNotice('مبلغ پیشنهاد باید بزرگ‌تر از صفر باشد.');
    const validUntilDate = cargoOffer.validUntil ? new Date(cargoOffer.validUntil) : new Date(Date.now() + 24 * 60 * 60 * 1000);
    if (Number.isNaN(validUntilDate.getTime())) return setNotice('تاریخ اعتبار پیشنهاد معتبر نیست.');
    const validUntil = validUntilDate.toISOString();
    try {
      const result = await write(`/api/cargo-inquiries/${cargoOffer.item.publicId}/offers`, {
        amount,
        currency: cargoOffer.currency || 'EUR',
        validUntil,
        basis: cargoOffer.basis,
        included: cargoOffer.included,
        excluded: cargoOffer.excluded,
        conditions: cargoOffer.conditions
      }, { key: `cargo-offer-${cargoOffer.item.publicId}-${cargoOffer.item.versionNo}` });
      if (result) { setCargoOffer(null); setNotice(result.message || 'پیشنهاد استعلام ثبت شد.'); await refresh(); }
    } catch (error) { setNotice(error.message); }
  };
  ...
  return <div className="driver-mobile-shell" dir="rtl">
    ...
    <main className="driver-mobile-main">
      ...
      {tab === 'opportunities' && <section className="driver-mobile-page"><SectionTitle eyebrow="استعلام عمومی حمل" title="فرصت‌های قیمت‌گذاری" action={<button className="driver-mobile-icon-button" type="button" onClick={refresh} aria-label="به‌روزرسانی"><Icon name="refresh" size={20} /></button>} />{cargoOpportunities.length ? cargoOpportunities.map((item) => <article className="driver-mobile-opportunity driver-mobile-opportunity--cargo" key={`${item.publicId}-${item.versionNo}`}><div className="driver-mobile-card-top"><Pill value={item.ownOffer?.state || 'NEW'} active={Boolean(item.ownOffer)} /><small>تا {dateText(item.deadlineAt)}</small></div><strong>{item.route?.origin?.city || 'مبدأ نامشخص'} ← {item.route?.destination?.city || 'مقصد نامشخص'}</strong><p>{item.cargo?.title || 'محموله'} · {item.cargo?.totalGrossWeightKg || '—'} کیلوگرم{item.cargo?.totalVolumeM3 ? ` · ${item.cargo.totalVolumeM3} مترمکعب` : ''}</p><span>{item.cargo?.specialFlags?.length ? `ویژگی‌ها: ${item.cargo.specialFlags.join('، ')}` : 'اطلاعات حساس در محدوده مجاز نمایش داده می‌شود.'}</span><div className="driver-mobile-card-actions">{item.ownOffer && <b>{Number(item.ownOffer.amount).toLocaleString('fa-IR')} {item.ownOffer.currency}</b>}<button type="button" onClick={() => setCargoOffer({ item, amount: item.ownOffer?.amount || '', currency: item.ownOffer?.currency || 'EUR', validUntil: '', basis: '', included: '', excluded: '', conditions: '' })}>{item.ownOffer ? 'ویرایش پیشنهاد' : 'ثبت پیشنهاد'}</button></div></article>) : <Empty title="استعلام عمومی در دسترس نیست" detail="فقط فرصت‌های منتشرشده برای راننده واجد شرایط نمایش داده می‌شود." />}<SectionTitle eyebrow="داخل سازمان شرکت Y" title="فرصت‌های بار" />{opportunities.length ? opportunities.map((item) => <article className="driver-mobile-opportunity" key={`${item.rfqId}-${item.yOrgId}`}><div className="driver-mobile-card-top"><Pill value={item.ownBid?.state || 'NEW'} active={Boolean(item.ownBid)} /><small>تا {dateText(item.deadlineAt)}</small></div><strong>{item.route?.origin} ← {item.route?.destination}</strong><p>{item.cargo?.type || 'محموله'} · {item.cargo?.weight || '—'} {item.cargo?.unit || ''}</p><span>شرکت Y: {item.yName}</span><div className="driver-mobile-card-actions">{item.ownBid && <b>{Number(item.ownBid.amount).toLocaleString('fa-IR')} {item.ownBid.currency}</b>}<button type="button" onClick={() => { setBid(item); setBidForm({ amount: item.ownBid?.amount || '', currency: item.ownBid?.currency || 'EUR', note: '', submit: false }); }}>ثبت پیشنهاد داخلی</button></div></article>) : <Empty title="فرصت داخلی در دسترس نیست" detail="فرصت فقط با پوشش معتبر و دعوت همان شرکت Y نمایش داده می‌شود." />}</section>}
    ...
    <nav className="driver-mobile-nav" aria-label="ناوبری اصلی راننده"><NavButton active={tab === 'home'} onClick={() => selectDriverTab('home')} icon="home" text="خانه" /><NavButton active={tab === 'opportunities'} onClick={() => selectDriverTab('opportunities')} icon="rfq" text="پیشنهادها" badge={opportunities.length + cargoOpportunities.length} /><NavButton active={tab === 'trips'} onClick={() => selectDriverTab('trips')} icon="route" text="سفرهای من" /><NavButton active={tab === 'documents'} onClick={() => selectDriverTab('documents')} icon="document" text="مدارک" /><NavButton active={tab === 'account'} onClick={() => selectDriverTab('account')} icon="user" text="حساب" /></nav>
    {cargoOffer && <div className="driver-mobile-modal-backdrop"><section className="driver-mobile-modal"><span className="platform-eyebrow">استعلام خودکار حمل</span><h2>پیشنهاد قیمت</h2><p>{cargoOffer.item.route?.origin?.city || 'مبدأ'} ← {cargoOffer.item.route?.destination?.city || 'مقصد'}</p><label><span>مبلغ</span><input inputMode="decimal" value={cargoOffer.amount} onChange={(event) => setCargoOffer((current) => ({ ...current, amount: event.target.value }))} /></label><label><span>ارز</span><input maxLength={3} value={cargoOffer.currency} onChange={(event) => setCargoOffer((current) => ({ ...current, currency: event.target.value.toUpperCase() }))} /></label><label><span>اعتبار تا (اختیاری)</span><input type="datetime-local" value={cargoOffer.validUntil} onChange={(event) => setCargoOffer((current) => ({ ...current, validUntil: event.target.value }))} /></label><label><span>مبنای قیمت</span><input value={cargoOffer.basis} onChange={(event) => setCargoOffer((current) => ({ ...current, basis: event.target.value }))} placeholder="مثلاً کرایه کامل مسیر" /></label><textarea value={cargoOffer.included} onChange={(event) => setCargoOffer((current) => ({ ...current, included: event.target.value }))} placeholder="موارد شامل" /><textarea value={cargoOffer.excluded} onChange={(event) => setCargoOffer((current) => ({ ...current, excluded: event.target.value }))} placeholder="موارد خارج از قیمت" /><textarea value={cargoOffer.conditions} onChange={(event) => setCargoOffer((current) => ({ ...current, conditions: event.target.value }))} placeholder="شرایط اجرا" /><div className="driver-mobile-modal-actions"><button type="button" onClick={() => setCargoOffer(null)}>انصراف</button><button className="driver-mobile-primary" type="button" onClick={submitCargoOffer}>ثبت پیشنهاد</button></div></section></div>}
