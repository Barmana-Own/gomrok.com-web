...
export default function DriverMobilePanel({ user, token, apiUrl, onLogout }) {
  ...
  return <div className="driver-mobile-shell" dir="rtl">
    <header className="driver-mobile-header"><div className="driver-mobile-brand"><span>✓</span><div><strong>GOMROK</strong><small>اپ راننده · سفر امن</small></div></div><div className="driver-mobile-header-actions"><span className={`driver-mobile-connect${offline ? ' is-offline' : ''}`}><i />{offline ? 'آفلاین' : 'آنلاین'}</span><button type="button" onClick={onLogout}>خروج</button></div></header>
    ...
    <nav className="driver-mobile-nav"><NavButton active={tab === 'home'} onClick={() => setTab('home')} icon="⌂" text="خانه" /><NavButton active={tab === 'opportunities'} onClick={() => setTab('opportunities')} icon="◇" text="پیشنهادها" badge={opportunities.length} /><NavButton active={tab === 'trips'} onClick={() => setTab('trips')} icon="⇢" text="سفرهای من" /><NavButton active={tab === 'documents'} onClick={() => setTab('documents')} icon="▤" text="مدارک" /><NavButton active={tab === 'account'} onClick={() => setTab('account')} icon="◉" text="حساب" /></nav>
...
function NavButton({ active, onClick, icon, text, badge }) { return <button className={active ? 'is-active' : ''} type="button" onClick={onClick}><b>{icon}</b><span>{text}</span>{badge ? <i>{badge}</i> : null}</button>; }
